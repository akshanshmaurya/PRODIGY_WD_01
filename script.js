...
let startTime;
let lapCount = 1;
let lapTimes = []; // Declare lapTimes here

function startStop() {
 if (!isRunning) {
    isRunning = true;
    document.getElementById("startStop").textContent = "Stop";
    // Correctly calculate startTime based on the current state
...
function lap() {
 if (isRunning) {
    ...
    lapCount++;
    // Reset startTime for the next lap
    startTime = Date.now();
 }
}
